...
GPIO.output(22, True)
GPIO.output(27, True)
# Can play with this value so to give a more random timing effect
# time between pre-chimes
wait = .75
wait_longer = 1.0
# Wait this long between the pre-chimes and the hour gongs
wait_gong = 2.0
...
# bell should register the time, correct to within one second per day. So, at
# twelve o'clock, for example, it is the first of the twelve hour-bell strikes
# that signifies the hour.
...
sleep(5)
# don't gong till 6am on weekdays
if day_of_week > 0 and day_of_week < 6 and gongs_24h > 5 and gongs_24h < 21:
    strobe_blue_lights(must_strobe_for)
    chime()
# don't gong till 10am in weekends
elif ((day_of_week == 0 or day_of_week == 6) and
        gongs_24h > 9 and gongs_24h < 21):
    strobe_blue_lights(must_strobe_for)
    chime()
